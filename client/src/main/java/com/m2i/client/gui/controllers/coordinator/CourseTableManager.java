...
public class CourseTableManager {
    ...
    private MFXTableColumn<CourseResponseDTO> createActionsColumn() {
        ...
        actionsColumn.setRowCellFactory(course -> new MFXTableRowCell<CourseResponseDTO, String>(s -> "") {
            ...
            private final HBox actions = new HBox(10, viewBtn, downloadBtn, deleteBtn);

            {
                // Initialize buttons
                setupActionButton(viewBtn, FontAwesomeSolid.EYE, "#4caf50");
                setupActionButton(downloadBtn, FontAwesomeSolid.DOWNLOAD, "#2196f3");
                setupActionButton(deleteBtn, FontAwesomeSolid.TRASH, "#f44336");
    ...
    public void loadData() {
        CompletableFuture.supplyAsync(() -> {
            try {
                return coordService.getAllCourses(SessionManager.getInstance().getCurrentSession().getSessionId());
            } catch (Exception e) {
    ...
    private void deleteCourse(CourseResponseDTO course) {
        if (UIUtils.showConfirmation("Delete Course",
                String.format("Are you sure you want to delete course: %s?", course.getName()))) {
            try {
                coordService.deleteCourse(SessionManager.getInstance().getCurrentSession().getSessionId(), course.getCourseId());
                courses.remove(course);
            } catch (Exception e) {
                UIUtils.showError("Error deleting course", e.getMessage());
            }
        }
    }
}