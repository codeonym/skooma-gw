...
public class CourseTableManager {
    ...
    private MFXTableColumn<CourseResponseDTO> createActionsColumn() {
        ...
        actionsColumn.setRowCellFactory(course -> new MFXTableRowCell<CourseResponseDTO, String>(s -> "") {
            ...
            private final MFXButton uploadBtn = new MFXButton("");
            private final HBox actions = new HBox(10, uploadBtn, viewBtn, downloadBtn);

            {
                // Initialize buttons
                setupActionButton(viewBtn, FontAwesomeSolid.EYE, "#eee");
                setupActionButton(downloadBtn, FontAwesomeSolid.DOWNLOAD, "#eee");
                setupActionButton(uploadBtn, FontAwesomeSolid.UPLOAD, "#eee");
    ...
    public void loadData() {
        CompletableFuture.supplyAsync(() -> {
            try {
                return teacherService.getAssignedCourses(SessionManager.getInstance().getCurrentSession().getSessionId());
            } catch (Exception e) {
